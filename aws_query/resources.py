...
def run_raw_operation(service, region, operation):
...
class Resources(object):
    ...
    def __str__(self):
        ...
        # opdesc = '{} {}'.format(self.service, self.region)
    ...
    def acquire(cls, service, region, operation):
        ...
        response = run_raw_operation(service, region, operation)
    ...
    def resources(self):
        ...
        if self.service == 'kms' and self.operation == 'ListKeys':
            list_aliases = run_raw_operation(self.service, self.region, 'ListAliases')
            service_key_ids = [
        ...
        # Remove ecs/DescribeCapacityProviders
        if self.service == 'ecs' and self.operation == 'DescribeCapacityProviders':
            del response['DescribeCapacityProviders']
        ...
        if self.service == 'ec2' and self.operation == 'DescribeInternetGateways':
            describe_vpcs = run_raw_operation(self.service, self.region, 'DescribeVpcs')
